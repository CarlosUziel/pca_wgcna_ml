...
DATA_ROOT: Path = STORAGE.joinpath(f"PCTA_{GOI_SYMBOL}")
...
SAMPLE_CLUSTER_CONTRAST_LEVELS: Iterable[
    ...
    [
        ({SAMPLE_CONTRAST_FACTOR: (test,)}, {SAMPLE_CONTRAST_FACTOR: (control,)})
        for test, control in [("prim", "norm"), ("met", "prim")]
    ]
    + [
        (
...
if __name__ == "__main__":
    ...
    if PARALLEL and len(input_collection) > 1:
        parallelize_map(
            ...
            input_collection,
            threads=user_args["threads"] // WGCNA_THREADS,
        )
    else:
