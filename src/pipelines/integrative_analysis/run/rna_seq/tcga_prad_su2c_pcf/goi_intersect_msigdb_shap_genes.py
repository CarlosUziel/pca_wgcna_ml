...
from pipelines.integrative_analysis.utils import intersect_msigdb_shap_genes
...
for contrast_comparison, contrast_comparison_filters in CONTRAST_COMPARISONS.items():
    ...
    for p_col, p_th, lfc_level, lfc_th, classifier_name, shap_th, msigdb_cat in product(
        P_COLS, P_THS, LFC_LEVELS, LFC_THS, CLASSIFIER_NAMES, SHAP_THS, MSIGDB_CATS
    ):
...
if __name__ == "__main__":
    ...
    if PARALLEL and len(input_collection) > 1:
        parallelize_map(
            functools.partial(run_func_dict, func=intersect_msigdb_shap_genes),
            input_collection,
            threads=user_args["threads"],
        )
    else:
        for ins in tqdm(input_collection):
            intersect_msigdb_shap_genes(**ins)
