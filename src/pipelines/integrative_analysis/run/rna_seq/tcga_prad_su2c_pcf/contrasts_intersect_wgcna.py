...
from pipelines.integrative_analysis.utils import intersect_wgcna
...
for contrast_comparison, contrast_comparison_filters in CONTRAST_COMPARISONS.items():
    ...
    for p_col, p_th, lfc_level, lfc_th, correlation_type, network_type in product(
        P_COLS, P_THS, LFC_LEVELS, LFC_THS, CORRELATION_TYPES, NETWORK_TYPES
    ):
...
if __name__ == "__main__":
    ...
    if PARALLEL and len(input_collection) > 1:
        parallelize_map(
            functools.partial(run_func_dict, func=intersect_wgcna),
            input_collection,
