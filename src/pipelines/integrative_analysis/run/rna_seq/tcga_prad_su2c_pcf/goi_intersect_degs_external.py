...
from pipelines.integrative_analysis.utils import intersect_degs_external
...
EXTERNAL_GENES_PATH: Path = STORAGE.joinpath(
    "Andi_primary_PSMA_high_vs_PSMA_low_diff_dep_new.txt"
)
...
CONTRAST_COMPARISONS: Dict[
    ...
    "comparison_2": (
        (
            {SAMPLE_CONTRAST_FACTOR: ["met"], f"{GOI_LEVEL_PREFIX}_20": ["high"]},
            {SAMPLE_CONTRAST_FACTOR: ["met"], f"{GOI_LEVEL_PREFIX}_20": ["low"]},
        ),
    ),
}
...
external_genes_df = pd.read_csv(
    EXTERNAL_GENES_PATH,
    sep="\t",
)
external_genes = {
    "val_ml_sig": set(
        external_genes_df[external_genes_df["pos_vs_neg_p.adj"] < 0.05]["name"]
    ),
}
    ...
    for p_col, p_th, lfc_level, lfc_th in product(P_COLS, P_THS, LFC_LEVELS, LFC_THS):
        ...
        input_collection.append(
            dict(
                ...
                root_path=DATA_ROOT,
                comparison_alias=f"{contrast_comparison}_andi",
                p_col=p_col,
                p_th=p_th,
                lfc_level=lfc_level,
                lfc_th=lfc_th,
...
# 3. Run DEGs intersection analysis
if __name__ == "__main__":
    freeze_support()
    if PARALLEL and len(input_collection) > 1:
        parallelize_map(
            functools.partial(run_func_dict, func=intersect_degs_external),
            input_collection,
            threads=user_args["threads"],
        )
    else:
        for ins in tqdm(input_collection):
            intersect_degs_external(**ins)
